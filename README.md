...
| SmolVLA | LeRobot, `v0.6.1` (`7e241bd6`) | [`models/smolvla`](models/smolvla/foldquant_integration/README.md) | float · W8A8 · W4A4 | yes | _pending_ |
...
Two absences are worth naming. SmolVLA has no cascade arm by design, not by
omission — cascade needs a PyTorch fake-quant of the quantized LLM, and the one
here is validated for Qwen2 / Qwen3 / Qwen3-VL and Gemma; it refuses SmolLM2
rather than emulating a convention it has not been checked against. The float
arm is still missing from N1.5 and π₀.₅ only because it has not been built
yet: `--llm-scheme float` traces the module through the deployed forward
