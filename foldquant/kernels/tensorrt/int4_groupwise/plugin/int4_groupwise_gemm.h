 ...
 */
void gemv_forward_cuda_new(__nv_bfloat16 const* in_feats, int8_t const* kernel, half const* scaling_factors, __nv_bfloat16* out_feats,
 ...
 * @param group_size Quantization group size
 * @param stream CUDA stream
 * @throws std::runtime_error if group size is unsupported
 */
void gemm_forward_cuda_new(__nv_bfloat16 const* in_feats, int8_t const* kernel, half const* scaling_factors, __nv_bfloat16* out_feats,
    int m, int n, int k, int group_size, cudaStream_t stream);
} // namespace kernel
} // namespace trt_edgellm
