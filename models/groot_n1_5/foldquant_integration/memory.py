...
logger = logging.getLogger("foldquant.groot_n1_5.memory")
...
_MECHANISM = "never-materialize: checkpoint loaded on CPU, replaced LLM decoder layers/norm/lm_head and the DiT withheld as meta, remaining components moved to CUDA"


def _load(args, device: str):
    from . import calibration as C

    policy = C.load_policy(args.model_path, args.embodiment_tag, device, denoising_steps=args.denoising_steps)
...
def _install(policy, root, args):
    ...
    if "llm" in installed.engines:
        lm = R.llm_module(policy)
        inner = getattr(lm, "model", lm)
        replaced += [inner.layers, inner.norm]
        if getattr(lm, "lm_head", None) is not None:
            replaced.append(lm.lm_head)
    if "dit" in installed.engines:
        replaced.append(R.dit_module(policy))
